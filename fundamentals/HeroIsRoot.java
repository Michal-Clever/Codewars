...
public class HeroIsRoot {

    public static long IntRac(long n, long guess) {
        int counter = 0;
        for (long prevGuess = 0; (guess - prevGuess) * (guess - prevGuess) >= 1; counter++) {
            prevGuess = guess;
...
//        named after the first-century Greek mathematician Hero of Alexandria who gave the first description of the method.
//        Hero's method can be obtained from Newton's method which came 16 centuries after.
//
//        We approximate the square root of a number n by taking an initial guess x,
//        an error e and repeatedly calculating a new approximate integer value x using: (x + n / x) / 2;
